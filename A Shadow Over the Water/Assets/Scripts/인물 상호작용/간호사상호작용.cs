...
public class 간호사상호작용 : MonoBehaviour
{
	public Text 간단표기제목;
	public Text 간단표기내용;
	...
	public void 멜리사대화()
    {
		...
		if (DataBaseManager.시간대 == 3)
		{
			...
			else
			{
				if (숫자 == 1)
				{
					인터렉션컨트롤러.간호사1_1일차저녁_반복대화1();

				}

				else if (숫자 == 2)
				{
					인터렉션컨트롤러.간호사1_1일차저녁_반복대화2();
        ...
        else
        {
			...
			else
			{
				if (숫자 == 1)
				{
					인터렉션컨트롤러.간호사1_1일차낮_반복대화1();
				}
				else if (숫자 == 2)
				{
					인터렉션컨트롤러.간호사1_1일차낮_반복대화2();
				}
	...
	public void 알버트키워드()
	{
		...
		if (DataBaseManager.시간대 == 3)
		{
			...
			{
				인터렉션컨트롤러.간호사1_1일차저녁_불면증키워드대화();
			}
			else if (DataBaseManager.키워드하위 == "편집증 유행")
			{
				인터렉션컨트롤러.간호사1_1일차저녁_편집증키워드대화();
			}
			else if (DataBaseManager.키워드하위 == "비릿한 냄새 소문")
			{
				인터렉션컨트롤러.간호사1_1일차저녁_비릿한냄새키워드대화();
			}
			else
			{
				인터렉션컨트롤러.간호사1_1일차저녁_그외키워드대화();
			}
		}
        else
        {
