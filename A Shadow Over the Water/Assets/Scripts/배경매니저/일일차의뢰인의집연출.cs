...
public class 일일차의뢰인의집연출: MonoBehaviour
{    //연출시 못움직이게하는
    ...
    public void 최초로의뢰인의집입장()
    {
        if(DataBaseManager.최초의뢰인집 == false)
        {
            플레이어애니메이션.SetFloat("move", 0);
            플레이어애니메이션.SetFloat("walk", 0);
            플레이어애니메이션.SetFloat("run", 0);
            주인공.transform.localScale = new Vector3(0.8249288f, 0.8249288f, 1.40338f);
            DataBaseManager.최초의뢰인집 = true;
            DataBaseManager.연출중움직임제한 = true;

            StartCoroutine(n초후연출시작());
        }
    }
    ...
    public void 안방나가는연출()
    {
        ...
        StartCoroutine(n초후연출시작2());

    }
    IEnumerator n초후연출시작2()
    {
        yield return new WaitForSeconds(3f);
        DataBaseManager.연출중움직임제한 = false;
        인터렉션컨트롤러.노아독백();
