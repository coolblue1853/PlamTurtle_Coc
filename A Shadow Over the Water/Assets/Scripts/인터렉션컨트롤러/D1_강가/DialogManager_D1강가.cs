...
public class DialogManager_D1강가 : MonoBehaviour
{
    ...
    */
    ...
    public selectionUIManager_D1강가 selectionUIManager;
    ...
    void 텍스트체커()
    {
        ...
        if (isDialog)
        {
            if (isNext)
            {
                ...
                if (Input.GetKeyDown(KeyCode.E) && isSelectButton == false && DataBaseManager.연출진행중 == false && DataBaseManager.옵션창여부 == false)
                {
                    z키로넘기기여부2 = false;
                    z키로넘기기여부 = false;
                    isNext = false;
